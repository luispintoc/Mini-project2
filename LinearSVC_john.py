...
pipeline = Pipeline([
    ('features_union', FeatureUnion([
                ('ngrams_feature', Pipeline([('ngrams_vect', TfidfVectorizer(ngram_range=(1,3), binary = True))
            ])),
                ('words_feature', Pipeline([('words_vect', CountVectorizer(min_df = 2, vocabulary = vocab, binary = True))
            ])),
    ...
    ('reduce_dim', None),
    ('classifier', LinearSVC(max_iter = 9000))])
    #('classifier', SVC(gamma='scale'))])
    #('classifier', LogisticRegression(penalty = 'l2', max_iter = 6000))])
...
parameters_grid = { #'classifier__solver': ('lbfgs','newton-cg'),
                    ...
                    # 'reduce_dim__score_func':(f_classif(),chi2()),
                    'reduce_dim__k':(20000,15000,10000)}
